...
class PlayerController:
    ...
    def get_future_reduction(self, board, depth, enemy=True):
        """
        Look ahead depth moves, compute the total reduction in the enemy's valid moves. Returns the maximum cumulative reduction found.
        """
        if depth == 0:
            return 0
        best_reduction = 0
        current_enemy_moves = self.count_valid_moves(board, enemy=enemy)
        for move in board.get_possible_directions():
            if board.is_valid_move(move):
                next_board, success = board.forecast_action(move)
                if not success:
                    continue
                reduction = current_enemy_moves - self.count_valid_moves(next_board, enemy=enemy)
                reduction += self.get_future_reduction(next_board, depth - 1, enemy=enemy)
    ...
    def find_trap_sequence(self, b: player_board.PlayerBoard):
        ...
        while total_cost <= b.get_length() - b.get_min_player_size():
            ...
            # we have found a move that we are for sure going to make
            current_cost += 1
            ...
            for move in best_moves:
                ...
                next_board, success = board_copy.forecast_action(move)
                if success:
                    # Look ahead 'self.search_depth' hops (minus 1 hop since we've already taken one move)
                    future_reduction = self.get_future_reduction(next_board, self.search_depth - 1, enemy=True)
            ...
            current_cost += 1  # Cost increases by 1 for each move
            total_cost += current_cost
