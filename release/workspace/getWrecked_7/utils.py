...
def get_move_to_nearest_apple(b: player_board.PlayerBoard, danger_list=[]):
    # format: x, y, snake current direction, initial move leading to this cell
    q = Queue(dim=4)
    # print("b possible actions")
    directions = sorted(b.get_possible_directions(),
                        key=lambda move: move in danger_list)
    for initial_move in directions:  # enqueue all possible cells you can move to
...
def get_best_move_spaces(b: player_board.PlayerBoard, danger_list=[]):
    best_move = None
    best_open = -1
    ...
    for initial_move in directions:
